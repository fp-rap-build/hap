...
const { getAllDocuments, createDocument, updateDocument } = require('./documents/controllers');
...
const { sendPromiseToPayEmail, sendConfirmationOfApproval } = require('../../utils/sendGrid/messages');
...
router.route('/:id/address').all(validateRequestId).put(updateActivity, updateAddress);
...
router.route('/:id/payments').all(validateRequestId).post(updateActivity, sendPayment);
